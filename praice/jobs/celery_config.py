...
app.conf.beat_schedule = {
    "collect-yfinance-headlines": {
        "task": "praice.jobs.tasks.collect_headlines_by_source_job",
        "schedule": crontab(minute="*/65"),
        "args": ("yfinance",),
    },
    "collect-articles": {
        "task": "praice.jobs.tasks.collect_articles_job",
        "schedule": crontab(minute="*/101"),
    },
    "collect-price-data": {
        "task": "praice.jobs.tasks.collect_price_data_job",
        "schedule": crontab(minute="0", hour="22"),  # daily at 6:00 PM ET
    },
    "calculate-store-technical-analysis": {
        "task": "praice.jobs.tasks.calculate_and_store_technical_analysis_job",
        "schedule": crontab(minute="30", hour="22"),  # daily at 6:00 PM ET
    },
    "collect-store-fundamental-data": {
        "task": "praice.jobs.tasks.collect_and_store_fundamental_data_job",
        "schedule": crontab(
            minute="0", hour="23", day_of_month="1"
        ),  # monthly on the 1st day at 7:00 PM ET
    },
    "populate-news-words-count": {
        "task": "praice.jobs.tasks.populate_news_words_count_job",
    ...
    "populate-relevance-score": {
        "task": "praice.jobs.tasks.populate_relevance_score_job",
